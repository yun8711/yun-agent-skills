...
本 Skill 目录内、与 `SKILL.md` 同级的 `config.json`（安装后通常为 `~/.agents/skills/llm-wiki-helper/config.json`）
...
2. 用户确认后，**写入** `config.json`（含 `notesRoot`、`templatesDir` 与可选 `configuredAt`；勿提交到公共仓库；仓库根 `.gitignore` 已忽略 `skills/llm-wiki-helper/config.json`）。
