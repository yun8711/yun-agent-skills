...
生成 docx 时 Read 已安装的 `docx` Skill（`npx skills add anthropics/skills --skill docx`）中「Creating New Documents」；版面见 [references/docx-layout.md](references/docx-layout.md)。优先用 `scripts/export_source_docx.py`。
