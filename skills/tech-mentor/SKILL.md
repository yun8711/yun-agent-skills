...
- **读取默认偏好**：AI 必须首先尝试读取本 Skill 目录下的 `defaults.json`，并在对应问题中作为默认建议展示（与下文「默认值持久化机制」一致）。
...
**持久化规则**：`tech_background`、`learning_style`、`workspace`（默认路径与命名规则）持久化到本 Skill 目录下的 `defaults.json`。其余字段通常不跨课程持久化。
...
AI 在用户明确提供常用偏好后，应**自动创建或更新**本 Skill 目录下的 `defaults.json`。
